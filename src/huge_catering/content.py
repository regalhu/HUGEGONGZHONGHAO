...
def _ten_lessons_advices_for_plan(plan: TopicPlan) -> list[Advice]:
    ...
    angle = plan.angle
    rows = [
        (f"{theme}先算账", f"{theme}不是凭感觉做。先把销量、毛利、人工和损耗摊开，账清楚了，动作才不会乱。"),
        (f"{angle}要有表", f"{angle}最怕喊得响、没人做。今天就做一张检查表，谁负责、几点查、查什么，全写清楚。"),
        (f"{theme}只抓一个主动作", f"别一口气改十件事。围绕{theme}先抓一个最赚钱或最漏钱的动作，小店改得少，反而见效快。"),
        (f"{theme}要让员工会复述", f"老板懂不算懂，员工会说会做才算数。把{theme}拆成一句话术和一个动作，班前会就能讲。"),
        (f"{theme}得让顾客看见", f"再好的调整，顾客看不见也白搭。把{theme}变成菜单推荐、服务提醒或门店细节，让顾客有感知。"),
        (f"{theme}别拿打折遮", f"打折能来人，也能吃利润。{theme}要先查体验、出品和复购理由，别把优惠当万能胶。"),
        (f"{angle}每天看三个数", f"围绕{angle}，每天只盯三个数：客流、毛利、复购。数变了，说明动作有效；数不变，就别自我感动。"),
        (f"{theme}先小试再放大", f"新动作先用一天、一个班次、一组员工试。小范围跑通了，再放大，别拿整家店冒险。"),
        (f"{theme}少救火多定规则", f"{theme}反复出问题，多半不是人不行，是规则不清。把问题写进流程，明天就少一次救火。"),
        (f"{theme}稳住才有后劲", f"餐饮赚钱不是一锤子买卖。把{theme}做稳，少亏一点、多留一点，店才有后劲。"),
    ]
    return [Advice(index=index, title=title, body=body) for index, (title, body) in enumerate(rows, start=1)]
...
def _hot_sections_for_plan(plan: TopicPlan) -> list[Advice]:
    ...
    return [
        Advice(1, f"{plan.angle}：这个热点为什么和老板有关", f"{keyword}表面是流量话题，背后是顾客选择标准在变化。本篇只看{plan.angle}，避免和其他草稿重复写泛泛的客流、毛利、复购。"),
        Advice(2, f"{plan.topic}影响哪几笔钱", f"先拆五笔账：客流能不能进来，客单会不会被压低，毛利会不会变薄，复购能不能留下，风险会不会放大。{plan.topic}最关键的是别把热闹误判成利润。"),
        Advice(3, f"围绕{plan.angle}今天做这3件事", actions),
        Advice(4, f"胡哥总结：从{plan.angle}看{keyword}", f"{keyword}会过去，但{plan.angle}里的问题会留下。餐饮老板今天先改一个动作、看一个数字、定一个标准，比跟着喊热闹更值钱。"),
    ]
...
def _methodology_sections_for_plan(plan: TopicPlan) -> list[Advice]:
    problem = plan.topic
    return [
        Advice(1, f"{problem}为什么发生", f"{problem}，通常不是老板不努力，而是动作太散。菜单、员工、顾客、数据各管各的，最后就变成忙得很热闹，利润没留下。"),
        Advice(2, f"{plan.angle}里的老板误区", f"第一，以为多上新就能解决。第二，以为多打折就能解决。第三，以为员工催一催就能解决。其实{plan.angle}要靠标准和复盘，不靠一时兴奋。"),
        Advice(3, f"解决{problem}的4步动作", f"第一步，把问题拆成一个数字，比如客单、毛利、复购或出餐时长。第二步，选一个最小动作先试一天。第三步，写成班前会话术和检查表。第四步，晚上只看结果，不看感觉。"),
        Advice(4, f"{problem}检查清单", f"今天就查：有没有负责人，有没有一句话术，有没有数据记录，有没有顾客反馈，有没有第二天继续执行的标准。五项过三项，{problem}就能开始变清楚。"),
        Advice(5, f"胡哥总结：{problem}别靠感觉扛", f"方法论不是把文章写漂亮，是让门店明天能少乱一点。围绕{problem}，先做小动作，再看真数据，利润就不会天天失踪。"),
    ]
